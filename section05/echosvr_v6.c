...
handle_msg_from_io(int fd, short int event,void* data) {
   ...
   while(1){
     ...
     conn = connection_get(ctx->hash,flowid,&is_new);
     if (conn && is_new) {
       printf("new connection, flowid=%u\n",flowid);
     }
...
main(int argc, char **argv) {
  ...
  hash = (hashbase_t *)malloc(sizeof(hashbase_t));
  if (!hash) {
    printf("Couldn't create hash\n");
    return 1;
  }
  connection_init(hash);
